...
for (modality in names(experimental_predictors_day_0)) {
  # modality <- "pbmc_cell_frequency"
  feature_list[[paste0(modality)]] <- experimental_predictors_day_0[[modality]]
  feature_list[[paste0(modality, "+metadata+baseline")]] <- experimental_predictors_day_0[[modality]] %>%
    dplyr::left_join(meta_data_cov, by="subject_id")
  feature_list[[paste0(modality, "_pca")]] <- experimental_predictors_day_0_pca[[modality]]
  feature_list[[paste0(modality, "_pca+metadata+baseline")]] <- experimental_predictors_day_0_pca[[modality]] %>%
    dplyr::left_join(meta_data_cov, by="subject_id")
}
#names(feature_list)
...
for (task in task_meta) {
  ...
  for (model_name in c("rf", "lm", "lasso")) {
    # model_name <- "lasso"
...
result_summary_2 <- result_summary %>%
  dplyr::group_by(task, model, feature_set) %>%
  dplyr::summarise(mean_test_srho=mean(test_srho),
                   sd_test_rsho=sd(test_srho), 
                   .groups = "drop")
```
