...
experimental_data <- 
  experimental_data[-which(names(experimental_data) == "pbmc_gene_expression_only_corrected")]

experimental_predictiors <- generate_wide_experimental_data(experimental_data=experimental_data,
                                                            impute="median", 
                                                            verbose=TRUE)

experimental_predictiors <- 
  experimental_predictiors[-which(names(experimental_data) == "pbmc_gene_expression")]
...
for (task in task_meta) {
  ...
  for (modality in names(experimental_predictiors)) {
    #modality <- names(experimental_predictiors)[3]
    
    cat(paste0("\n\n### ", modality, "\n\n"))
    
    modality_data <- experimental_predictiors[[modality]] %>%
      as.data.frame() %>% 
    ...
    colnames(modality_data) <- stringr::str_replace_all(colnames(modality_data), "-", "_")
    colnames(modality_data) <- stringr::str_replace_all(colnames(modality_data), "\\+", "_")
    colnames(modality_data) <- stringr::str_replace_all(colnames(modality_data), "/", "_")
