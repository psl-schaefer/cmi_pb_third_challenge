...
outliers_list <- purrr::set_names(names(specimen_per_day)) %>%
  purrr::map(~ purrr::set_names(names(experimental_data)) %>%
               purrr::map(~list()))
outliers_list$day_0$plasma_cytokine_concentration_by_legendplex <- c(661)
...
for (day in names(specimen_per_day)) {
  ...
  for (assay in names(experimental_data)) {
    # assay <- names(experimental_data)[2]
    ...
    if (nrow(specimen_df_filtered) == 0) {
      cat(paste0("No specimen for this combination of assay and day available"))
    } else {
      ...
      if (assay == "pbmc_gene_expression") {
        ...
        ensemble_to_gene <- tibble(versioned_ensembl_gene_id = colnames(assay_mtx_wide)) %>%
        ...
        assay_mtx_wide <- assay_mtx_wide[ , ensemble_to_gene$versioned_ensembl_gene_id]
        colnames(assay_mtx_wide) <- ensemble_to_gene$gene_symbol
      ...
      if ( any( rownames(assay_mtx_wide) %in% outliers_list[[day]][[assay]] ) ) {
        no_outerliers <- rownames(assay_mtx_wide)[!rownames(assay_mtx_wide) %in% outliers_list[[day]][[assay]]]
        assay_mtx_wide <- assay_mtx_wide[no_outerliers, ]
      }
