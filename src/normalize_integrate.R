...
normalize_experimental_data <- function(meta_data, 
  ...
  normalized_data$pbmc_gene_expression_raw <- pbmc_gene_expression_wide %>%
    as.data.frame() %>%
    tibble::rownames_to_column(var="specimen_id") %>%
    tidyr::pivot_longer(cols=!c(specimen_id), 
                        names_to=feature_col,
  ...
  stopifnot(dplyr::setequal(names(experimental_data), 
                            names(normalized_data)[!names(normalized_data) %in% c("pbmc_gene_expression_raw")]))
  
  return(normalized_data)
}


integrate_experimental_data <- function(meta_data, experimental_data) {
  integrated_data <- list()
  
  # 1) pbmc_cell_frequency: Nothing
  integrated_data$pbmc_cell_frequency <-
    experimental_data$pbmc_cell_frequency
  ...
  integrated_data$pbmc_gene_expression_only_corrected <- 
